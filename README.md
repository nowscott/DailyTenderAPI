...
它把原本容易堆在快捷指令里的日期计算、阳历/农历生日倒计时、城市提取、每日短句和文本拼接放到服务端处理。手机端只负责采集当天的数据，然后把 JSON 发给接口；接口返回一段可以直接发送的 `message`。
...
- 导入链接：`https://www.icloud.com/shortcuts/3626b8be64b74d0e9ed80a85d3317a0f`
...
`birthday` 默认按阳历 `MM-DD` 计算。农历生日仍然写农历月日，但额外加 `calendar: "lunar"`；如果是农历闰月生日，再加 `leapMonth: true`。
...
| `people[].birthday` | 是 | `MM-DD` | 生日；默认阳历，农历生日也写农历月日。 |
| `people[].calendar` | 否 | `lunar` | 只影响生日倒计时；不传就是公历，传 `lunar` 才按农历计算。 |
| `people[].leapMonth` | 否 | 布尔 | 农历闰月生日传 `true`。 |
...
| `person1Calendar` / `person2Calendar` | 否 | `lunar` | 扁平字段模式下的生日历法；不传就是公历。 |
| `person1LeapMonth` / `person2LeapMonth` | 否 | 布尔 | 扁平字段模式下的农历闰月标记。 |
...
| `date` | 实际参与计算的日期。 |
| `loveDays` | 恋爱天数。 |
| `people` | 两个人的生日倒计时、生日历法、下一次公历生日和 emoji。 |
| `quote` | 英文短句和中文翻译。 |
...
```text
GET /api/daily?date=2026-06-23&loveStart=2022-05-20&person1Name=Person%20A&person1Birthday=08-16&person2Name=Person%20B&person2Birthday=08-16&person2Calendar=lunar
```
...
| `person1Calendar` | 否 | `lunar` | 第一个人的生日历法；不传就是公历。 |
| `person1LeapMonth` | 否 | 布尔 | 第一个人的农历生日是否是闰月。 |
| `person2Name` | 是 | 文本 | 第二个人的称呼。 |
| `person2Birthday` | 是 | `MM-DD` | 第二个人的生日。 |
| `person2Calendar` | 否 | `lunar` | 第二个人的生日历法；不传就是公历。 |
| `person2LeapMonth` | 否 | 布尔 | 第二个人的农历生日是否是闰月。 |
| `timeZone` | 否 | IANA 时区 | 默认 `Asia/Shanghai`。 |
...
接口会校验 JSON body、日期、生日、农历生日字段和时区。比如 `null`、数组、字符串 JSON、非法 `timeZone` 都会返回 `400 invalid_request`，这样快捷指令端能明确知道是请求参数问题。
